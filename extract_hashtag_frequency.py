...
start_datetime = time_functions.return_datetime(timeinfo[0],time=timeinfo[1],setting="vs")
end_tweet = gzip.open(infiles[-1],"rb").readlines()[-1]
end_timeinfo = [end_tweet.split("\t")[date_column],end_tweet.split("\t")[time_column]]
end_datetime = time_functions.return_datetime(timeinfo[0],time=timeinfo[1],setting="vs")
timesegments = []
